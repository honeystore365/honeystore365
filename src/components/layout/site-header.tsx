...
// import { createClientComponent } from '@/lib/supabase/client'; // Keep for signOut - Removed
import { useSession } from '@/context/SessionProvider'; // Import useSession hook
import { createClient } from '@/lib/supabase/client';
import { ShoppingCart, User } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react'; // Keep useState, add useEffect back just in case, though likely not needed for session
...
const navLinkBaseClasses =
  'relative flex items-center gap-1.5 text-base font-medium text-gray-700 transition-colors duration-300 hover:text-honey';
...
export function SiteHeader() {
  ...
  const { session, loading } = useSession(); // Call useSession at the top level
        ...
        <div className='flex h-16 items-center justify-between w-full max-w-none px-4 sm:px-6 lg:px-8'>
          ...
          <div className='flex flex-shrink-0 items-center'>
            <Link href='/' className='flex items-center gap-2 md:gap-3'>
              ...
              />
              <span className='hidden text-lg font-bold text-honey-dark sm:inline md:text-xl'>{COMPANY_NAME}</span>
            </Link>
          </div>
      ...
      <div className='flex h-16 items-center justify-between w-full max-w-none px-4 sm:px-6 lg:px-8'>
          ...
          {navigationLinks.map(link => {
            ...
            return (
              <Link
                ...
                className={cn(navLinkBaseClasses, 'px-3 py-2 rounded-md hover:bg-gray-50')}
              >
        ...
        <div className='hidden flex-shrink-0 items-center gap-3 md:flex'>
          {session ? (
            ...
            authLinks.map(link => (
              <Button key={link.href} variant={link.variant} size='sm' className={link.extraClasses} asChild>
