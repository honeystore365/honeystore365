...
import { createOrder, getCustomerDetailsForCheckout } from '@/actions/checkoutActions';
...
export default function CartDisplayClient({ initialItems, initialTotal }: CartDisplayClientProps) {
  ...
  const handleProceedToPayment = async () => {
    ...
    const customerDetailsResult = await getCustomerDetailsForCheckout(session.user.id);
    
    if (!customerDetailsResult.success) {
      console.error('[CartDisplayClient] Error fetching customer details:', customerDetailsResult.error);
      toast({ title: 'خطأ', description: `فشل في جلب بيانات العميل: ${customerDetailsResult.error}`, variant: 'destructive' });
      setIsLoading(false);
      return;
    }

    const { customer, address } = customerDetailsResult.data || {};
    const customer_id = customer?.id;
    const first_name = customer?.first_name;
    const last_name = customer?.last_name;
    console.log('[CartDisplayClient] Customer details fetch result:', {
      customer_id,
      first_name,
      last_name,
      address,
    ...
    const orderResult = await createOrder(orderParamsData);
    console.log('[CartDisplayClient] Create order result:', orderResult);

    if (!orderResult.success || !orderResult.data) {
      console.error('[CartDisplayClient] Error creating order:', orderResult.error);
      toast({ title: 'خطأ في إنشاء الطلب', description: (typeof orderResult.error === 'string' ? orderResult.error : orderResult.error?.message) || 'حدث خطأ غير متوقع.', variant: 'destructive' });
      setIsLoading(false);
      return;
    }
  ...
  if (items.length === 0 && !isLoading) {
    return (
      <div className='container mx-auto py-10 px-4 text-center'>
        ...
        <p className='text-lg text-muted-foreground mb-6'>لم تقم بإضافة أي منتجات إلى سلتك بعد.</p>
        ...
        {items.map((item: CartItem) => {
          if (!item.product) {
            return (
              <div
                key={item.id}
                className='bg-white p-4 rounded-xl shadow-lg flex justify-between items-center border border-destructive'
              >
          ...
          return (
            <div
              key={item.id}
              className='bg-white p-4 sm:p-6 rounded-xl shadow-lg flex flex-col sm:flex-row items-center gap-4 sm:gap-6 transition-shadow hover:shadow-xl'
            >
              ...
              <div className='flex-grow text-center sm:text-right'>
                ...
                <p className='text-lg font-bold text-honey mt-2 sm:mt-1'>{item.product.price} د.ت</p>
              </div>
