// Server-side wrapper for admin auth functions
...
// Server-side admin auth functions for API routes
export class AdminAuthServerService {
  static async signInAdmin(email: string, password: string): Promise<AdminLoginResult> {
    try {
      // Vérifier d'abord si l'email est celui de l'admin
      if (!isAdminEmail(email)) {
        logger.warn('Admin login attempt with non-admin email', {
          component: 'AdminAuthServerService',
  ...
  static async getCurrentAdminSession() {
    try {
      const cookieStore = await cookies();
      const supabase = createServerClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL!,
