...
interface Order {
  ...
  shipping_address: { phone_number: string | null } | null; // Include phone number from shipping address
}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">#</th>{/* Sequential Order Number */}
              ...
              <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">رقم الهاتف</th> {/* New column for phone number */}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {orders.map((order, index) => {
              ...
              return (
              <tr key={order.id}><td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">{index + 1}</td> {/* Sequential Order Number */}
...
export default async function AdminPage() {
  ...
  // Note: Handle customerRpcError if needed
   ...
   .from('orders')
   .select('id, order_date, customer_id, total_amount, status, customers(first_name, last_name), shipping_address:addresses(phone_number)') // Select customer names and phone number from shipping address
